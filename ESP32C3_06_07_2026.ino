// ====================================================================
// ESP32-C3 FULL CODE - UPDATED WITH PATTERN BRIGHTNESS & SPEED CONFIG
// ====================================================================
...
// ================= GPIO (ESP32-C3 SAFE PINOUT - FIXED) =================
#define RELAY1 2
#define RELAY2 3
#define RELAY3 4
#define RELAY4 5
#define RELAY5 6
#define RELAY6 0            // FIXED: পিন কনফ্লিক্ট এড়াতে GPIO 0 ব্যবহার করা হয়েছে
#define AUTO_OFF_PIN 10      
#define RELAY_LOW_BEAM 1   
...
// ================= NEW: PATTERN BRIGHTNESS STATES =================
uint8_t frPatternBrightness = 100;  // Front/Rear প্যাটার্নের জন্য ব্রাইটনেস
uint8_t backPatternBrightness = 100;// Only Back প্যাটার্নের জন্য ব্রাইটনেস
...
// ================= PATTERNS FOR ONLY BACK (Size 11) =================
...
// ================= PATTERNS FOR FRONT & REAR (Size 10) =================
...
// ================= UPDATE PWM BRIGHTNESS DYNAMICALLY =================
void handleDynamicPWM() {
    if (signalActive) {
        ledcWrite(LEDPIN, map(brightness, 0, 100, 0, 255));
    } 
    else if (relay6State) {
        // Helmet Eye Fade Logic handles its own PWM inside handleRelay6Fade()
    } 
    else if (relay5State && (relay1State || relay3State)) {
        // Front and Rear Pattern Active
        ledcWrite(LEDPIN, map(frPatternBrightness, 0, 100, 0, 255));
    } 
    else if (relay4State && relay2State) {
        // Only Back Pattern Active
        ledcWrite(LEDPIN, map(backPatternBrightness, 0, 100, 0, 255));
    } 
    else {
        // Default System Brightness Slider
        ledcWrite(LEDPIN, map(brightness, 0, 100, 0, 255));
    }
}
...
void applyRelays(){
    ...
    handleDynamicPWM(); // আপডেটেড PWM ভোল্টেজ রিয়েল-টাইমে অ্যাপ্লাই হবে
}

// ================= EEPROM (FIXED SPACING FOR 128 BYTES) =================
void saveState(){
    ...
    EEPROM.write(30, lowBeamState);  
    EEPROM.write(31, selEyePattern); 

    // নতুন ভেরিয়েবল সেভ করা হচ্ছে
    EEPROM.write(32, frPatternBrightness);
    EEPROM.write(33, backPatternBrightness);
...
void loadState(){
    ...
    lowBeamState=EEPROM.read(30);    
    selEyePattern=EEPROM.read(31);   

    frPatternBrightness=EEPROM.read(32);
    backPatternBrightness=EEPROM.read(33);
    ...
    if(frPatternBrightness > 100) frPatternBrightness = 100;
    if(backPatternBrightness > 100) backPatternBrightness = 100;
...
void toggleAutoOff(){ autoOffState = !autoOffState; applyRelays(); saveState(); server.send(200, "text/plain", "OK"); }
void toggleLowBeam(){ lowBeamState = !lowBeamState; applyRelays(); saveState(); server.send(200, "text/plain", "OK"); }
...
void setPatternSpeedConfig(){
    ...
    if(server.hasArg("frb")) frPatternBrightness = constrain(server.arg("frb").toInt(), 0, 100);
    if(server.hasArg("bb")) backPatternBrightness = constrain(server.arg("bb").toInt(), 0, 100);
...
void checkSignal(){
    ...
    if (sig != lastSignal || waitingRestore) {
        applyRelays();
    }
    lastSignal = sig;
}
...
void runPattern(){
    ...
    if(currentMillis - lastBackPatternTime >= backPatternDelay){
        ...
        if(relay4State && relay2State) mustApply = true;
    }
    ...
    if(currentMillis - lastFRPatternTime >= frPatternDelay){
        ...
        if(relay5State && (relay1State || relay3State)) mustApply = true;
    }
...
void sendStatus(){
    ...
    json+=",\"frb\":"+String(frPatternBrightness);  // Added
    json+=",\"bb\":"+String(backPatternBrightness); // Added
    json        +="}";
...
<div id="sidePanel" class="side-panel">
   ...
   <div class="dropdown-box">
        ...
        <div class="speed-input-container">
            ...
            <input type="number" id="frBrightnessInput" min="0" max="100" placeholder="%">
        </div>
    </div>
    ...
    <div class="dropdown-box">
        ...
        <div class="speed-input-container">
            <span>Brightness (%)</span>
            <input type="number" id="backBrightnessInput" min="0" max="100" placeholder="%">
        </div>
        <button class="speed-submit-btn" onclick="saveSpeeds()">SET PATTERN CONFIG</button>
    </div>
...
// UPDATED: সেভ করার সময় ব্রাইটনেস ভ্যালুও পাঠানো হচ্ছে URL প্যারামিটারে
function saveSpeeds() {
    var bs = document.getElementById('backSpeedInput').value;
    var frs = document.getElementById('frSpeedInput').value;
    var frb = document.getElementById('frBrightnessInput').value;
    var bb = document.getElementById('backBrightnessInput').value;
    
    if(bs && frs && frb && bb) {
        fetch('/setPatternSpeed?bs=' + bs + '&frs=' + frs + '&frb=' + frb + '&bb=' + bb).then(() => { 
            alert("Pattern Configuration Saved!"); 
            up(); 
        });
    } else { alert("Please fill all speed and brightness fields!"); }
}
...
function up(){
    fetch('/status?t=' + new Date().getTime()).then(r=>r.json()).then(d=>{
        ...
        // শুধু যদি স্লাইডার ইউজার ড্র্যাগ না করে থাকে তখন ভ্যালু আপডেট হবে
        if(document.activeElement.id !== 'br') {
        ...
        // UPDATED: রিয়েল-টাইম ডাটা সিঙ্ক করার কোড
        if(document.activeElement.id !== 'frBrightnessInput') document.getElementById('frBrightnessInput').value = d.frb;
        if(document.activeElement.id !== 'backBrightnessInput') document.getElementById('backBrightnessInput').value = d.bb;
...
void setup(){
    Serial.begin(115200);
    EEPROM.begin(128); // INCREASED SIZE FOR PATTERN BRIGHTNESS TO 128 BYTES
